	...
	public void run() {
		...
		while (!isInterrupted()) {
			...
			if (j == 2) {
				...
				/*
				 * LinkedList<PendentMessage> pendentMessages =
				 * server.getPendentMessagesQueue();
				 * 
				 * for (PendentMessage m : pendentMessages) {
				 * 
				 * System.out.println("Message from: " + m.getMsg().getOrigin()
				 * + " to " + m.getMsg().getDestiny() + " with the text (" +
				 * m.getMsg().getText() +")");
				 * 
				 * }
				 */
