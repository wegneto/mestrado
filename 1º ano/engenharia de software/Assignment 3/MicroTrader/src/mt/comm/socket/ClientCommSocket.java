...
public class ClientCommSocket {
	 ...
	 */
	public ClientCommSocket(String host, String nickname, BlockingQueue<ClientSideMessage> clientMessages) throws UnknownHostException, IOException {
		this.host = host;
	...
	public void connect() {
		...
		if (socket != null && out != null && in != null) {
			...
			try {
				System.out.println("ClientComm >> Sending " + serverMsg);
...
class ClientCommThread extends Thread {
	 ...
	 *            server.
	 */
	public ClientCommThread(String nickame, Socket socket, ObjectInputStream in, BlockingQueue<ClientSideMessage> clientMessages) {
		...
		if (socket != null && in != null) {
			...
			while (isConnected) {
				try {
					...
					System.out.println("ClientComm >> Processing " + message);
					clientMessages.put(message);
				} catch (EOFException | SocketException e) {
					System.out.println(String.format("ClientComm >> The socket for client '%s' has been closed", nickname));
					isConnected = false;
