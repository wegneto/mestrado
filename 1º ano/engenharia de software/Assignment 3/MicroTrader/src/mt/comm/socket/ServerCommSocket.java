...
/**
 * This class responsible for handling sockets and streams between several
...
public class ServerCommSocket extends Thread {
	...
	/**
	 * Auxiliary structure a with information about connected users and its
	 * output stream, in order to be possible to send client messages for the
	 * matching nickname. This structure is used also to validate if a certain
	 * user, by its nickname, is or not connected.
	 */
	 ...
	 *             created.
	 */
	public ServerCommSocket(BlockingQueue<ServerSideMessage> serverMessages) throws IOException {
		...
		try {
			while (true) {
				...
				if (connectedUsers.get(nickname) != null) {
					...
					//Adding connected message to the queue in order to inform the server componenent.
...
class ServerCommThread extends Thread {
	...
	public ServerCommThread(String nickname, Socket clientSocket, ObjectInputStream in, BlockingQueue<ServerSideMessage> serverMessages, Map<String, ObjectOutputStream> connectedUsers) {
